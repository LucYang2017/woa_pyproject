...
def check_boundary_3d_tracking(position):
    while position[0] < X_MIN or position[0] > X_MAX:
        if position[0] < X_MIN:
            position[0] = X_MIN + abs(position[0] - X_MIN) + 0.05
        elif position[0] > X_MAX:
            position[0] = X_MAX - abs(position[0] - X_MAX) - 0.05

    while position[1] < Y_MIN or position[1] > Y_MAX:
        if position[1] < Y_MIN:
            position[1] = Y_MIN + abs(position[1] - Y_MIN) + 0.05
...
def check_boundary_3d_finding(position):
    while position[0] < X_MIN or position[0] > X_MAX:
        if position[0] < X_MIN:
            position[0] = X_MIN + abs(position[0] - X_MIN) + 0.05
        elif position[0] > X_MAX:
    ...
    while position[1] < Y_MIN or position[1] > Y_MAX:
        if position[1] < Y_MIN:
            ...
            position[1] = Y_MAX - abs(position[1] - Y_MAX) - 0.05

    while position[2] < Z_MIN or position[2] > Z_MAX:
        if position[2] < Z_MIN:
            position[2] = Z_MIN + abs(position[2] - Z_MIN) + 0.05
        elif position[2] > Z_MAX:
            position[2] = Z_MAX - abs(position[2] - Z_MAX) - 0.05
...
def go_forward(agents):
    for agent in agents:
        ...
        new_position = check_boundary_3d_finding(new_position)
        agent.position = new_position
    return agents


def go_forward_2d(agents, height):
    for agent in agents:
        if len(agent.history) <= 1:
            # gradient = np.array([round(random.uniform(X_MIN, X_MAX), 2), round(random.uniform(Y_MIN, Y_MAX), 2),
            #                      round(random.uniform(Z_MIN, Z_MAX), 2)])
